...
use std::{
    ...
    io::{BufRead, BufReader, BufWriter, Read, Write as IoWrite},
    path::{Path, PathBuf},
    process,
...
fn health_stats(connection: &Connection) -> Result<HealthStats> {
...
fn ensure_recall_events_session_column(transaction: &Transaction<'_>) -> Result<()> {
...
fn validate_recall_log_request(request: &RecallLogRequest) -> Result<()> {
...
fn is_supported_status(value: &str) -> bool {
    matches!(
        value,
        status::ACTIVE
            | status::SUPERSEDED
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct NormalizedSpaceCreateRequest {
    name: String,
...
fn validate_space_create_request(
    request: &SpaceCreateRequest,
...
fn normalize_required_space_component(name: &str, value: &str) -> Result<String> {
    let trimmed = value.trim();
...
fn normalize_space_name(value: Option<&str>) -> Result<Option<String>> {
    value
        .map(|value| normalize_required_space_component("space", value))
        .transpose()
}

fn normalize_optional_human_text(
    name: &str,
    value: Option<&str>,
    max_chars: usize,
) -> Result<Option<String>> {
...
fn space_exists(connection: &Connection, space: &str) -> Result<bool> {
...
fn ensure_space_exists(connection: &Connection, space: &str) -> Result<()> {
...
fn cleanup_vestigial_long_term_silo(connection: &Connection) -> Result<()> {
...
fn load_spaces(connection: &Connection, limit: usize) -> Result<Vec<SpaceRecord>> {
    let mut statement = connection.prepare(
        "SELECT
            s.name,
            s.display_name,
...
fn load_space(connection: &Connection, space: &str) -> Result<Option<SpaceRecord>> {
    connection
        .query_row(
            "SELECT
                s.name,
...
fn space_record_from_row(row: &Row<'_>) -> rusqlite::Result<SpaceRecord> {
    Ok(SpaceRecord {
        name: row.get(0)?,
...
fn load_silos(connection: &Connection, space: &str, limit: usize) -> Result<Vec<SiloRecord>> {
    let mut statement = connection.prepare(
        "SELECT
            si.space_name,
            si.name,
...
fn default_silo(connection: &Connection, space: &str) -> Result<Option<String>> {
    connection
        .query_row(
...
fn ensure_silo_exists(connection: &Connection, space: &str, silo: &str) -> Result<()> {
    let exists: i64 = connection.query_row(
...
fn now_timestamp(connection: &Connection) -> Result<String> {
    connection
...
fn is_supported_scope(value: &str) -> bool {
    matches!(
        value,
...
fn json_string_for_store(value: &str) -> String {
...
fn open_initialized_read_fast(path: &Path) -> Result<Connection> {
    validate_store_path(path)?;
    if !path.exists() {
        return Err(Error::NotInitialized {
            path: path.to_path_buf(),
...
fn open_initialized_write(path: &Path) -> Result<Connection> {
    validate_store_path(path)?;
    if !path.exists() {
        return Err(Error::NotInitialized {
            path: path.to_path_buf(),
...
fn reject_sqlite_sidecar_symlinks(path: &Path) -> Result<()> {
...
fn inspect_on_copy<T>(path: &Path, inspect: impl FnOnce(&Connection) -> Result<T>) -> Result<T> {
...
fn user_version(connection: &Connection) -> Result<i32> {
    connection
...
fn validate_initialized(path: &Path, connection: &Connection) -> Result<()> {
    let actual = user_version(connection)?;
    if actual == 0 {
        return Err(Error::NotInitialized {
            path: path.to_path_buf(),
...
fn required_config_value(path: &Path, connection: &Connection, key: &str) -> Result<String> {
...
fn space_names(connection: &Connection) -> Result<Vec<String>> {
...
fn space_stats(connection: &Connection) -> Result<Vec<SpaceStats>> {
    let mut statement = connection.prepare(
        "SELECT s.name,
