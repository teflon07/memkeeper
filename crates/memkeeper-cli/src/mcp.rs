...
fn tool_definitions() -> Vec<Value> {
    vec![
        tool(
            "stats",
            "Return memkeeper store statistics. Read-only. include_health adds a memory-governance rollup.",
            json!({
                "include_indexes": { "type": "boolean", "description": "Include per-index row counts." },
                "include_health": { "type": "boolean", "description": "Include the governance/health rollup." },
            }),
            &[],
        ),
        tool(
            "search",
            "Search memories: semantic-primary when embeddings are available, with deterministic BM25/FTS degradation. Reranked by default.",
            json!({
                "query": { "type": "string" },
                ...
                "include_source": { "type": "boolean", "description": "Reveal provenance (default false)." },
                "semantic_enabled": { "type": "boolean" },
                "rerank": { "type": "boolean", "description": "Cross-encoder rerank (default true)." },
            }),
            &["query"],
        ),
        tool(
        ...
        tool(
            ...
            json!({
                ...
                "max_edges": { "type": "integer", "description": "Default 50." },
        ...
        tool(
            ...
            json!({
                ...
                "depth": { "type": "integer", "description": "Default 1." },
                "max_edges": { "type": "integer", "description": "Default 50." },
                "max_memories": { "type": "integer", "description": "Default 10." },
        ...
        tool(
            ...
            json!({
                "max_memories": { "type": "integer", "description": "Default 1000." },
                "space": { "type": "string" },
            }),
            &[],
        ),
        tool(
            "remember",
            "Store one atomic, self-contained fact per call. One claim per call. Do not store secrets or transcripts. \
             source_type marks provenance (default assistant-inference; pass explicit-user when the user stated it). \
             mode controls resolution against same-key memories: auto|append|supersede|suggest|conflict.",
            json!({
                "content": { "type": "string" },
                "space": { "type": "string" },
                "silo": { "type": "string" },
                ...
                "derive_keys": { "type": "boolean", "description": "Auto-fill keys from content (default true)." },
                ...
                "mode": { "type": "string", "enum": ["auto", "append", "supersede", "suggest", "conflict"] },
                "source_type": { "type": "string", "description": "assistant-inference (default) or explicit-user." },
        ...
        tool(
            "forget",
            "Tombstone one specific memory id. Preserves audit history; not a hard delete. \
...
for factual corrections so the signal is captured explicitly rather than inferred later.",
            json!({
                "memory_id": { "type": "string" },
                "reason": { "type": "string" },
                "mode": { "type": "string", "enum": ["tombstone", "correct"] },
        ...
        tool(
            "verify",
            "Stamp a volatile memory as re-confirmed against ground truth now. Does NOT promote to durable. \
             If the value CHANGED, write a new memory and supersede instead.",
            json!({
                "memory_id": { "type": "string" },
                "verified_against": { "type": "string" },
            }),
        ...
        tool(
            "pack",
            "Build a compact, ready-to-inject context pack from one or more queries. Read-only.",
            json!({
                "queries": { "type": "array", "items": { "type": "string" } },
                "title": { "type": "string", "description": "Default \"context\"." },
                "max_memories": { "type": "integer", "description": "Default 10." },
        ...
        tool(
            "candidate_submit",
            "Submit a candidate memory for human review instead of writing it directly. Use for \
             plausible-but-unverified inferences. Approval/rejection is a human action (CLI/dashboard).",
            json!({
                "content": { "type": "string" },
