...
pub(crate) static COMMAND_SCHEMAS: &[CommandSchema] = &[
    ...
    CommandSchema {
        ...
        fields: &[
            ...
            opt("min_score", "float", Some("0.0"), "Drop memories below this score."),
            opt("cosine_gate", "float", Some("0.0"), "Query-level cosine OR-gate."),
            opt("rerank_candidates", "int", Some("0"), "Candidate pool for rerank (0 = off)."),
            opt("query_embeddings", "float[][]", None, "Precomputed per-query embeddings."),
            opt("query_expansion", "bool", Some("false"), "Deterministically add subqueries before retrieval."),
            opt("thread_expansion", "bool", Some("false"), "Add same-entity/same-claim neighbors to the rerank pool."),
            opt("graph_expansion", "bool", Some("false"), "Associative recall: graph-expand the rerank pool one hop from top seeds (hybrid_assoc_v0)."),
            opt("graph_within_entity_maxsim", "bool", Some("false"), "Experimental: select one memory per graph entity by first-query MaxSim; requires late-interaction tokens."),
            opt("max_query_variants", "int", Some("MAX_BATCH_QUERIES"), "Cap expanded query count."),
            opt("max_thread_seeds", "int", Some("3"), "Anchor count for same-thread expansion."),
            opt("max_thread_neighbors", "int", Some("3"), "Neighbor count per same-thread anchor."),
            opt("max_graph_seeds", "int", Some("3"), "Top-of-pool anchors used for graph expansion."),
            opt("max_graph_neighbors", "int", Some("5"), "Graph-reachable neighbors unioned into the pool (activation budget)."),
            opt("graph_decay", "float", Some("0.5"), "Per-hop activation decay for graph expansion."),
            opt("graph_rerank_slots", "int", Some("0"), "Reserve N pack slots for top-activation graph candidates (0 = recall-widening only)."),
            opt("graph_activation_floor", "float", Some("0.0"), "Minimum activation to claim a reserved graph rerank slot."),
        ],
    ...
    CommandSchema {
        ...
        fields: &[
            ...
            opt("query_expansion", "bool", Some("false"), "Deterministically add subqueries before retrieval."),
            opt("thread_expansion", "bool", Some("false"), "Add same-entity/same-claim candidates."),
            opt("graph_expansion", "bool", Some("false"), "Add one-hop relationship candidates."),
            opt("graph_within_entity_maxsim", "bool", Some("false"), "Experimental first-query MaxSim selection within graph entities."),
            opt("max_query_variants", "int", Some("MAX_BATCH_QUERIES"), "Cap expanded query count."),
            opt("max_thread_seeds", "int", Some("3"), "Thread-expansion seed count."),
            opt("max_thread_neighbors", "int", Some("3"), "Thread neighbors per seed."),
            opt("max_graph_seeds", "int", Some("3"), "Graph-expansion seed count."),
            opt("max_graph_neighbors", "int", Some("5"), "Graph neighbor budget."),
            opt("graph_decay", "float", Some("0.5"), "Graph activation decay."),
            opt("graph_rerank_slots", "int", Some("0"), "Accepted for exact request replay."),
            opt("graph_activation_floor", "float", Some("0.0"), "Accepted for exact request replay."),
        ],
